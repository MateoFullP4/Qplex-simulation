"""
This file is designed to visualize the magnetic field generated by the
asymmetric magnet configuration used in QPlex.

The script generates:

1. 2D slices of the magnetic field magnitude |B| in the XY, XZ, and YZ planes.
2. 1D profiles of |B| along the X, Y, and Z axes.

Conventions used in this file:

- Field magnitude is calculated as |B| = sqrt(Bx^2 + By^2 + Bz^2) in Tesla.
- Spatial coordinates are expressed in meters for computation and converted to mm for plotting.
- Slices are centered on the origin (0,0,0) unless otherwise specified.
- The grid resolution is adjustable via the `tx`, `ty`, `tz`, `lx`, and `lz` arrays.

Purpose:

- Provide a clear visual representation of the magnetic field landscape
- Identify gradients and symmetries along different planes
- Facilitate the understanding and optimization of the MOT setup

Dependencies:

- The magnetic configuration is imported from `configurations.magnetic_configuration`
  and must define a `magnetSet_asymmetric` object compatible with Magpylib.
- Matplotlib and NumPy are used for plotting and data handling.
